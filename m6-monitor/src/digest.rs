...
mod tests {
    ...
    fn version_drift_across_the_fleet_is_reported() {
        let d = build(
            &[
                reading("syd", "ok", Some(perf_at("1.4.0"))),
                reading("lon", "ok", Some(perf_at("1.3.0"))),
            ],
            &Thresholds::default(),
            now(),
        );
    ...
    fn a_uniform_fleet_reports_no_drift() {
        let d = build(
            &[
                reading("syd", "ok", Some(perf_at("1.4.0"))),
                reading("lon", "ok", Some(perf_at("1.4.0"))),
            ],
            &Thresholds::default(),
            now(),
        );
    ...
    fn a_node_that_cannot_report_its_version_is_not_agreement() {
        ...
        let drift: Vec<_> = d
            ...
            .collect();
        assert_eq!(drift.len(), 1, "{:?}", d.findings);
        assert!(drift[0].text.contains("lon unknown"), "{}", drift[0].text);
    }
    ...
    fn a_healthy_fleet_has_no_findings() {
        ...
        let d = build(
            &[reading("syd", "ok", Some(perf(host, vec![])))],
            &Thresholds::default(),
            now(),
        );
