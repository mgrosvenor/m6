...
traps so they cannot be re-learned:
  ...
  - **Analytics lives at a different path per role.** Origin writes to
    /var/www/example-site/logs/, the cache nodes to /var/www/m6-cache/logs/.
...
# reproduced: origin has measured a flat ~3.3us across hundreds of windows on an
...
#
# Seen against this fleet: zgrab (2026-09-12, Exchange probe on edge-b),
# libredtail-http (2026-09-12, encoded traversal on edge-a).
...
# maxima of 266ms on edge-a and 192ms on edge-b, neither of which reproduced in 12
...
lb = sh("for i in $(seq 1 5); do curl -sk -o /dev/null "
        ...
        "-H 'Host: example.com' https://127.0.0.1/capabilities; done")
...
def cache_headers():
    ...
    for path, expect in want.items():
        try:
            p = subprocess.run(["curl", "-sI", "https://example.com" + path],
    ...
    asset, got = None, ""
    try:
        p = subprocess.run(["curl", "-s", "https://example.com/capabilities"],
                           capture_output=True, text=True, timeout=30)
        m = re.search(r"(/assets/[A-Za-z0-9_./-]+\?v=[a-z0-9]+)", p.stdout)
        if m:
            asset = m.group(1)
            q = subprocess.run(["curl", "-sI", "https://example.com" + asset],
                               capture_output=True, text=True, timeout=30)
...
def main():
    ...
    faults, warnings = [], []
    print("\n" + "=" * 74)
    print("example.com health check   %s UTC   window=%dm"
    ...
    for node, r, err in results:
        ...
        for ua, v in sorted(genuine.items(), key=lambda x: -x[1]["n"]):
            ...
            # then a crawler that reached edge-a or edge-b appears here with the
