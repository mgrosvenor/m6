...
use m6_http_lib::http11::{make_tls_server_config, Http11Listener, RequestOutcome};
...
fn start_http11_server<F>(cert_path: &str, key_path: &str, handler: F) -> (u16, Arc<AtomicBool>)
where
...
fn http11_get_returns_200_with_body() {
    ...
    assert!(
        ...
        &response_str[response_str.len().saturating_sub(50)..]
    );
}
...
fn http11_path_and_headers_are_parsed() {
    ...
    assert!(
        response_str.contains("path=/some/path"),
        "path not parsed: {}",
        response_str
    );
    assert!(
        response_str.contains("method=GET"),
        "method not parsed: {}",
        response_str
    );
}
...
fn http11_version_field_is_http11() {
    ...
    assert!(
        response_str.ends_with("HTTP/1.1"),
        "expected version=HTTP/1.1 in body, got: {}",
        response_str
    );
}
...
fn http2_get_returns_200_with_body() {
    ...
    tls.write_all(&h2_frame(0x7, 0x0, 0, &goaway_payload))
        .unwrap();
    tls.flush().ok();
    ...
    while let Some((ftype, _flags, _sid, payload, next)) = next_h2_frame(&raw, pos) {
        ...
        match ftype {
            ...
            0x0 => {
                ...
                if body_bytes.windows(8).any(|w| w == b"hello h2") {
                    got_body = true;
                }
            }
            _ => {}
    ...
    assert!(
        got_200,
        "expected :status 200 (0x88) in response HEADERS; raw={:?}",
        &raw[..raw.len().min(64)]
    );
    assert!(
        got_body,
        "expected body 'hello h2'; got {:?}",
...
fn http2_version_field_is_http2() {
    ...
    assert_eq!(
        body_bytes,
        b"HTTP/2",
        "expected version HTTP/2 in body, got {:?}",
        String::from_utf8_lossy(&body_bytes)
    );
}
...
fn http3_get_returns_200_with_body() {
    ...
    let server_handle =
        thread::spawn(move || run_h3_server_once(server_udp, &cert_path, &key_path, stop2));
...
fn quic_flush(conn: &mut quiche::Connection, udp: &UdpSocket, out: &mut [u8]) {
    loop {
        match conn.send(out) {
            Ok((written, info)) => {
                let _ = udp.send_to(&out[..written], info.to);
            }
...
fn run_h3_client_once(server_addr: std::net::SocketAddr) -> Result<(u16, Vec<u8>), String> {
    ...
    loop {
        ...
        if conn.is_established() && h3_conn.is_none() {
            ...
            h3_conn = Some(
                quiche::h3::Connection::with_transport(&mut conn, &h3cfg)
