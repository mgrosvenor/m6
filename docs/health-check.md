...
example.com runs on **three nodes, not one**. Every part of this check runs
on all three unless it says otherwise, and the report carries a row per node.

| node | role | service | WireGuard | serves |
|---|---|---|---|---|
| `origin` | origin | `m6-http-origin` | `10.0.0.1` | AU visitors directly, plus every cache miss from edge-a and edge-b |
| `edge-a` | cache | `m6-http-cache` | `10.0.0.4` | EU visitors |
| `edge-b` | cache | `m6-http-cache` | `10.0.0.5` | US visitors |

## Why this file exists

The check was run against `origin` alone for several hours running. That is not a
...
1. **A cache node serves most requests from its own cache, and those requests
   ...
   traffic only. Crawlers, bursts and probes that edge-a or edge-b answered are
   invisible there.
2. **Each node writes its own analytics, to a different path.** Origin uses
   `/var/www/example-site/logs/analytics.ndjson`; the cache nodes use
...
**Rule: a number that only describes origin is not a fleet number, and a number
...
```sh
# origin
ssh root@origin.example.com "journalctl -u m6-http-origin --since '20 minutes ago' --no-pager -o cat \
  | sed 's/\x1b\[[0-9;]*m//g' \
  | sed -n 's/.*\(INFO\|WARN\|ERROR\) *\([a-z_0-9:]*\):.*/\2/p' | sort | uniq -c"
# edge-a and edge-b, same but -u m6-http-cache
```
...
give the fix (`systemctl restart m6-http-origin` on origin, `m6-http-cache` on
edge-a/edge-b), and do not report "all clear" for that node.
...
| steady-state hit rate | ~1.0000 | **wrong as a fleet target.** True only of a freshly warmed page hit repeatedly. Real 24h rates: origin 0.56, edge-a 0.16, edge-b 0.19. |
...
| node | analytics path |
|---|---|
| origin | `/var/www/example-site/logs/analytics.ndjson` |
| edge-a, edge-b | `/var/www/m6-cache/logs/analytics.ndjson` |
...
3. Services to check: origin `m6-http-origin`, `m6-html`, `m6-file`,
   `render-contact`, `render-analytics`, `m6-http-redirect`; edge-a/edge-b
   `m6-http-cache`, `m6-http-redirect`.
4. `render-contact` (origin only) for `rejected` / `message sent` / `SMTP`.
...
Read **all three** analytics files, not origin's alone. A crawler that edge-a or
edge-b answered from cache never reaches origin and appears only in that node's
own file.
