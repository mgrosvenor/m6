...
2. **Neither cache node records analytics at all.** No `analytics` target in
   their journals, no `analytics.ndjson` on disk. So the traffic origin cannot see
...
`/var/www/example-site/logs/analytics.ndjson` (origin only, it is the only
...
1. The node's own journal, `journalctl -k | grep -i ufw`, and origin's
   `analytics.ndjson`.
...
- **Cache nodes record no analytics.** Anything edge-a or edge-b served from cache is
