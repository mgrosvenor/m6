...
//! figure is directly comparable to the `http/1.1/*` channel on m6-http's
...
fn main() {
    ...
    let cfg = probe::tls_config(b"http/1.1");
    ...
    for _ in 0..n {
        match probe::tls_handshake(&addr, cfg.clone()) {
            ...
            Ok((d, alpn)) if alpn.as_deref() == Some(b"http/1.1".as_slice()) => samples.push(d),
            Ok((_, alpn)) => {
                failures += 1;
                eprintln!(
    ...
    probe::report(
        "h1 rustls handshake (excludes TCP connect)",
        samples,
        failures,
    );
