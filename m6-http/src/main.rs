...
mod www_redirect_tests {
    ...
    fn redirects_www_to_apex_preserving_path_and_query() {
        ...
        assert_eq!(
            www_redirect_location(Some("www.example.com"), "/capabilities", Some("a=1&b=2"), &c),
    ...
    fn host_matching_is_case_insensitive_and_port_tolerant() {
        let c = cfg("example.com", true);
        for host in ["WWW.example.com", "Www.MGrosvenor.Com", "www.example.com:80"] {
            assert_eq!(
    ...
    fn apex_is_left_alone() {
        let c = cfg("example.com", true);
        assert_eq!(www_redirect_location(Some("example.com"), "/", None, &c), None);
    }
    ...
    fn other_hosts_are_left_alone() {
        let c = cfg("example.com", true);
        for host in ["origin.example.com", "edge-a.example.com", "evil.example", "www.evil.example"] {
            assert_eq!(
    ...
    fn disabled_by_config_and_absent_host() {
        assert_eq!(
            www_redirect_location(Some("www.example.com"), "/", None, &cfg("example.com", false)),
            None
        );
        assert_eq!(www_redirect_location(None, "/", None, &cfg("example.com", true)), None);
    }
